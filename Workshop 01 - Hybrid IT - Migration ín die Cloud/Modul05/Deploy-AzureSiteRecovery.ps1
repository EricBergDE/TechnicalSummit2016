...
Param(
	[string] $SubscriptionName = "Visual Studio Enterprise mit MSDN (MVP)",
	[string] $ASRVaultName = "IgniteDemoASRVault02",
	[string] $ResourceGroupName = "IgniteDemoASR02",
	[string] $Location = "East US",
	[string] $HVSiteName = "Ignite-HV02",
	[string] $RegKeyPath = "C:\Demo\",
	[string] $source = "https://aka.ms/downloaddra",
	[string] $destination = "C:\Demo\AzureSiteRecoveryProvider.exe",
	[string] $storageaccount = "igniteasrdemostor00001",
	[string] $ReplicationFrequencyInSeconds = "300",
	[string] $PolicyName = “IgniteRP”,
	[int] $Recoverypoints = 3,
	[string] $VMFriendlyName01 = "NanoServer01",
	[string] $VMFriendlyName02 = "NanoServer02",
...
#create Hyper-V Site
#old: $HVSiteJob = New-AzureRmSiteRecoverySite -Name $HVSiteName
$HVSiteJob = New-AzureRmSiteRecoveryFabric -Type HyperVSite -Name $HVSiteName
...
#old: $SiteIdentifier = Get-AzureRmSiteRecoverySite -Name $HVSiteName | Select -ExpandProperty SiteIdentifier
$SiteIdentifier = Get-AzureRmSiteRecoveryFabric -Name $HVSiteName | Select -ExpandProperty SiteIdentifier
...
do
{
    #old: $HVServerState = Get-AzureRmSiteRecoveryServer -FriendlyName $HVSiteName | select -ExpandProperty Connected
    $HVFabric = Get-AzureRmSiteRecoveryFabric -Name $HVSiteName
    $HVServerState = Get-AzureRmSiteRecoveryServicesProvider -Fabric $HVFabric | select -ExpandProperty Connected
}
...
#protectioncontainer - going to be deprecated
$protectionContainer = Get-AzureRmSiteRecoveryProtectionContainer

#associate
$Policy = Get-AzureRmSiteRecoveryPolicy -FriendlyName $PolicyName
#old: $associationJob  = Start-AzureRmSiteRecoveryPolicyAssociationJob -Policy $Policy -PrimaryProtectionContainer $protectionContainer
New-AzureRmSiteRecoveryProtectionContainerMapping -Name "Mapping" -PrimaryProtectionContainer $protectionContainer -Policy $Policy

Start-Sleep -Seconds 60

$Mapping = Get-AzureRmSiteRecoveryProtectionContainerMapping -Name "Mapping" -ProtectionContainer $protectionContainer
